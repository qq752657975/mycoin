...
func ExchangeOrderAdd(redisCli *redis.Redis, cli *database.KafkaClient, orderRpc eclient.Order, db *mydb.MsDB) {
	...
	for {
		...
		_ = json.Unmarshal(kafkaData.Data, &orderAdd)
		...
		if acquire {
			...
			err := transaction.Action(func(conn mydb.DbConn) error {
...
func ExchangeOrderComplete(redisCli *redis.Redis, cli *database.KafkaClient, db *mydb.MsDB) {
	//先接收消息
	for {
		kafkaData := cli.Read()
		var exchangeOrder *ExchangeOrder
		_ = json.Unmarshal(kafkaData.Data, &exchangeOrder)
		...
		if acquire {
			...
			if exchangeOrder.Direction == BUY {
				...
				if exchangeOrder.Type == MarketPrice {
					//市价买 amount USDT 冻结的钱  exchangeOrder.turnover扣的钱 还回去的钱 amount-exchangeOrder.turnover
					...
					//限价买 冻结的钱是 exchangeOrder.price*amount  成交了turnover 还回去的钱 exchangeOrder.price*amount-exchangeOrder.turnover
				...
				//卖 不管是市价还是限价 都是卖的 BTC  解冻amount 得到的钱是 exchangeOrder.turnover
				coinWallet, err := walletDomain.FindWalletByMemIdAndCoin(ctx, exchangeOrder.MemberId, exchangeOrder.CoinSymbol)
				if err != nil {
					logx.Error(err)
					cli.Rput(kafkaData)
