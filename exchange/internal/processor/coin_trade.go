...
func (c *CoinTradeFactory) Init(marketRpc mclient.Market, cli *database.KafkaClient, db *mydb.MsDB) {
	//初始化的操作
...
func NewCoinTrade(symbol string, cli *database.KafkaClient, db *mydb.MsDB) *CoinTrade {
	c := &CoinTrade{
		symbol:      symbol,
		kafkaClient: cli,
		db:          db,
